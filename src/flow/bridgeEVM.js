...
// Flow Wallet - mainnet Script  batchBridgeNFTToEvmV2 - v2.63
// Extension-2.7.8
...
access(all) fun mustCall(
    ...
    assert(res.status == EVM.Status.successful,
        message: "Failed to call '".concat(functionSig)
            .concat("error code: ").concat(res.errorCode.toString())
            .concat("error message: ").concat(res.errorMessage)
            .concat("gas used: ").concat(res.gasUsed.toString())
            .concat("caller address: 0x").concat(coa.address().toString())
            .concat("contract address: 0x").concat(contractAddr.toString())
    )
