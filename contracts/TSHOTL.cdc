...
access(all) contract TSHOTL: FungibleToken {
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        /// Called when this TSHOTL vault is burned via the `Burner.burn()` method
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                TSHOTL.totalSupply = TSHOTL.totalSupply - self.balance
            }
            self.balance = 0.0
        }
        ...
        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @TSHOTL.Vault
    ...
    access(all) fun createEmptyVault(vaultType: Type): @TSHOTL.Vault {
        return <-create Vault(balance: 0.0)
    }
    ...
    access(all) resource Admin {
        ...
        access(AdminEntitlement) fun mintTokens(amount: UFix64): @TSHOTL.Vault {
        TSHOTL.totalSupply = TSHOTL.totalSupply + amount
    ...
    // Burn tokens
    //
    // $TSHOTL token will be burned in exchange for underlying $FLOW when a user requests to unstake from the liquid staking protocol.
    // Note: the burned tokens are automatically subtracted from the total supply in the Vault destructor.
    access(account) fun burnTokens(from: @TSHOTL.Vault) {
    ...
    init() {
        ...
        self.tokenReceiverPath = /public/TSHOTLTokenReceiver
        self.tokenBalancePath = /public/TSHOTLTokenBalance

        // Put a new Collection in storage
        self.account.storage.save<@Admin>(<- create Admin(), to: /storage/TSHOTLAdmin)
