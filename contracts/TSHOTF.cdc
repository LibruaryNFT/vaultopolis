...
access(all) contract TSHOTF: FungibleToken {
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        /// Called when this TSHOTF vault is burned via the `Burner.burn()` method
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                TSHOTF.totalSupply = TSHOTF.totalSupply - self.balance
            }
            self.balance = 0.0
        }
        ...
        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @TSHOTF.Vault
    ...
    access(all) fun createEmptyVault(vaultType: Type): @TSHOTF.Vault {
        return <-create Vault(balance: 0.0)
    }
    ...
    access(all) resource Admin {
        ...
        access(AdminEntitlement) fun mintTokens(amount: UFix64): @TSHOTF.Vault {
        TSHOTF.totalSupply = TSHOTF.totalSupply + amount
    ...
    // Burn tokens
    //
    // $TSHOTF token will be burned in exchange for underlying $FLOW when a user requests to unstake from the liquid staking protocol.
    // Note: the burned tokens are automatically subtracted from the total supply in the Vault destructor.
    access(account) fun burnTokens(from: @TSHOTF.Vault) {
    ...
    init() {
        ...
        self.tokenReceiverPath = /public/TSHOTFTokenReceiver
        self.tokenBalancePath = /public/TSHOTFTokenBalance

        // Put a new Collection in storage
        self.account.storage.save<@Admin>(<- create Admin(), to: /storage/TSHOTFAdmin)
