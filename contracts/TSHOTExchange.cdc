...
access(all) contract TSHOTExchange {
   ...
   // Function to validate if an NFT is eligible for TSHOT (common or fandom tier)
    ...
    nftIDs.length > 0: "Cannot swap! No NFTs provided."
        ...
        while nftIDs.length > 0 {
            ...
            if !self.validateNFT(nft: &nft as &TopShot.NFT) {
                panic("NFT tier is not valid for TSHOT. Only common or fandom-tier moments are allowed.")
            }
...
while counter < Int(tokenAmount) {
    ...
    init() {
        ...
        self.ReceiptStoragePath = StoragePath(identifier: "TSHOTReceipt_".concat(self.account.address.toString()))!
    }
}
